...
class PlatformIndex extends Component
{
    ...
    protected PlatformRepositoryInterface $platforms;

    public function boot(PlatformRepositoryInterface $platforms): void
    {
    ...
    {
        if ($this->platforms->hasDeletedStatus($this->bulkSelectedIds)) {
    ...
    {
        try {
            $platform = $this->platforms->findForAdminOrFail((int) $rowId);
