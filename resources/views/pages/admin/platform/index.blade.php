<div>
    @section('pageTitle', __('admin.titles.manage_platforms'))

    @push('breadcrumbs')
        <x-partials.dashboard.breadcrumb :items="[
            ['label' => __('admin.nav.catalog_keys'), 'url' => 'javascript:void(0)'],
            ['label' => __('admin.nav.attributes'), 'url' => 'javascript:void(0)'],
            ['label' => __('admin.nav.platforms'), 'url' => 'javascript:void(0)'],
        ]" />
    @endpush
    ...
    <div class="bg-white dark:bg-slate-800 shadow  rounded-md w-full relative">
        ...
        <div class="flex-auto p-4">
            <livewire:admin.table.platform.platform-table />
        </div>
    </div>

    <x-reusable.modal wire:model="showCreateModal" :title="__('admin.modal.create_platform')" max-width="2xl">
        <form id="createPlatformForm" wire:submit="createPlatform">
            <div class="space-y-6">
                <div>
                    <h3 class="text-sm font-medium text-slate-900 dark:text-slate-100 pb-2 border-b border-slate-200 dark:border-slate-700">{{ __('admin.common.basic_information') }}</h3>
                    <div class="space-y-4 pt-4">
                        <div>
                            <label for="name" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('admin.common.name') }} <span class="text-red-500">*</span></label>
                            <input wire:model="createForm.name" type="text" id="name"
                                   ...
                                   placeholder="{{ __('admin.placeholders.platform_name') }}" required aria-required="true">
                            @error('createForm.name')
                                <p class="mt-1.5 text-xs text-red-600 dark:text-red-400" role="alert">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            <label for="slug" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('admin.common.slug') }}</label>
                            <input wire:model="createForm.slug" type="text" id="slug"
                                   class="w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 dark:border-slate-600 dark:focus:border-primary-500 transition-colors"
                                   placeholder="{{ __('admin.placeholders.slug_auto') }}">
                            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ __('admin.common.leave_blank_slug') }}</p>
                ...
                <div>
                    <h3 class="text-sm font-medium text-slate-900 dark:text-slate-100 pb-2 border-b border-slate-200 dark:border-slate-700">{{ __('admin.common.visual_links') }}</h3>
                    <div class="space-y-4 pt-4">
                        <div>
                            <label for="create_icon_file" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('admin.common.icon') }}</label>
                            ...
                            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ __('admin.common.icon_help') }}</p>
                            @if($createForm->icon_file)
                                <div class="mt-2 flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700">
                                    <img src="{{ $createForm->icon_file->temporaryUrl() }}"
                                         alt="{{ __('admin.common.icon_preview') }}"
                        ...
                        <div>
                            <label for="create_base_url" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('admin.common.base_url') }}</label>
                            <input wire:model="createForm.base_url" type="url" id="create_base_url"
                                   class="w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 dark:border-slate-600 dark:focus:border-primary-500 transition-colors"
                                   placeholder="https://example.com">
                            @error('createForm.base_url')
                                <p class="mt-1.5 text-xs text-red-600 dark:text-red-400" role="alert">{{ $message }}</p>
                            @enderror
                        </div>
                    </div>
            ...
            <div class="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-slate-200 dark:border-slate-700">
                <button wire:target="createPlatform" type="submit"
                    ...
                    <span wire:loading wire:target="createPlatform">{{ __('admin.common.saving') }}</span>
                    <span wire:loading.remove wire:target="createPlatform">{{ __('admin.common.submit') }}</span>
                </button>
    ...
    <x-reusable.modal wire:model="showViewModal" :title="__('admin.modal.platform_details')" max-width="2xl">
        @if($viewData)
            <div class="space-y-4">
                <div class="bg-slate-50 dark:bg-slate-900 rounded-lg p-4 text-sm border border-slate-200 dark:border-slate-700">
                    <dl class="divide-y divide-slate-200 dark:divide-slate-700">
                        ...
                        <div class="grid grid-cols-3 gap-4 py-3">
                            <dt class="font-medium text-slate-500 dark:text-slate-400">{{ __('admin.common.name') }}</dt>
                            <dd class="col-span-2 text-slate-900 dark:text-white">{{ $viewData['name'] }}</dd>
                        </div>

                        <div class="grid grid-cols-3 gap-4 py-3">
                            <dt class="font-medium text-slate-500 dark:text-slate-400">{{ __('admin.common.slug') }}</dt>
                            <dd class="col-span-2 text-slate-900 dark:text-white">{{ $viewData['slug'] }}</dd>
                        </div>

                        <div class="grid grid-cols-3 gap-4 py-3">
                            <dt class="font-medium text-slate-500 dark:text-slate-400">{{ __('admin.common.icon') }}</dt>
                            <dd class="col-span-2 flex items-center gap-3">
                                @if($viewData['icon_path'])
                                    <img src="{{ \App\Utilities\StorageUtility::getUrl($viewData['icon_path']) }}"
                                         alt="{{ $viewData['name'] ?? 'Platform' }} icon"
                                ...
                                @else
                                    <span class="text-sm text-slate-400 dark:text-slate-500 italic">{{ __('admin.common.no_icon_set') }}</span>
                                @endif
                            </dd>
                        </div>

                        <div class="grid grid-cols-3 gap-4 py-3">
                            <dt class="font-medium text-slate-500 dark:text-slate-400">{{ __('admin.common.base_url') }}</dt>
                            <dd class="col-span-2 text-slate-900 dark:text-white">{{ $viewData['base_url'] ?? '-' }}</dd>
                        </div>
    ...
    <x-reusable.modal wire:model="showEditModal" :title="__('admin.modal.edit_platform', ['id' => $editForm->platform?->id])" max-width="2xl">
        <form id="editPlatformForm" wire:submit="updatePlatform">
            <div class="space-y-6">
                <div>
                    <h3 class="text-sm font-medium text-slate-900 dark:text-slate-100 pb-2 border-b border-slate-200 dark:border-slate-700">{{ __('admin.common.basic_information') }}</h3>
                    <div class="space-y-4 pt-4">
                        <div>
                            <label for="edit_name" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('admin.common.name') }} <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label for="edit_slug" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('admin.common.slug') }}</label>
                ...
                <div>
                    ...
                    <div class="space-y-4 pt-4">
                        <div>
                            <label for="edit_icon_file" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('admin.common.icon') }}</label>
                            ...
                            @elseif($editForm->icon_path)
                                <div class="mt-2 flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-700">
                                    <img src="{{ \App\Utilities\StorageUtility::getUrl($editForm->icon_path) }}"
                                         alt="{{ $editForm->name ?? 'Platform' }} icon"
                                         class="w-12 h-12 object-contain"
                                         onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
                        ...
                        <div>
                            <label for="edit_base_url" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('admin.common.base_url') }}</label>
                            <input wire:model="editForm.base_url" type="url" id="edit_base_url"
                                   class="w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 dark:border-slate-600 dark:focus:border-primary-500 transition-colors"
                                   placeholder="https://example.com">
                            @error('editForm.base_url')
                                <p class="mt-1.5 text-xs text-red-600 dark:text-red-400" role="alert">{{ $message }}</p>
                            @enderror
                        </div>
                    </div>
                ...
                <div>
                    <h3 class="text-sm font-medium text-slate-900 dark:text-slate-100 pb-2 border-b border-slate-200 dark:border-slate-700">{{ __('admin.common.status') }}</h3>
                    <div class="pt-4">
                        <label for="edit_status" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{{ __('admin.common.status') }} <span class="text-red-500">*</span></label>
            ...
            <div class="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-slate-200 dark:border-slate-700">
                <button wire:target="updatePlatform" type="submit"
                        class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors focus:ring-2 focus:ring-blue-500/50 disabled:opacity-50">
                    <span wire:loading wire:target="updatePlatform">{{ __('admin.common.saving') }}</span>
                    <span wire:loading.remove wire:target="updatePlatform">{{ __('admin.common.update') }}</span>
                </button>
    ...
    <x-reusable.modal wire:model="showBulkStatusModal" :title="__('admin.modal.change_platform_status')" max-width="md">
        <form id="bulkStatusForm" class="space-y-4" wire:submit="bulkChangeStatusPlatform">

            <div class="mb-2">
                <label for="bulk_status" class="font-medium text-sm text-slate-600 dark:text-slate-400">
                    {{ __('admin.common.select_new_status') }} <span class="text-red-400">*</span>
                </label>
            ...
            <div class="flex items-center justify-end space-x-2 mt-6">
                <button wire:target="bulkChangeStatusPlatform" type="submit" class="inline-block focus:outline-none text-yellow-600 hover:bg-yellow-500 hover:text-white bg-transparent border border-yellow-400 dark:border-yellow-600 text-sm font-medium py-1 px-3 rounded mb-1 transition-colors">
                    {{ __('admin.common.apply_status') }}
                </button>
    ...
    @push('scripts')
        <x-admin.swal-listener table-name="platformTable" />
    @endpush
</div>
