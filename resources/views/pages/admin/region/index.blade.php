<div>
    @section('pageTitle', __('admin.titles.manage_regions'))
    ...
    @push('breadcrumbs')
        <x-partials.dashboard.breadcrumb :items="[
            ...
            ['label' => __('admin.nav.regions'), 'url' => 'javascript:void(0)'],
        ]" />
    @endpush
    ...
    <x-reusable.modal wire:model="showCreateModal" :title="__('admin.modal.create_region')" max-width="2xl">
        <form id="createRegionForm" class="space-y-4" wire:submit="createRegion">
            <div class="mb-2">
                ...
                <input wire:model="createForm.name" type="text" id="name" class="form-input w-full rounded-md mt-1 border border-slate-300/60 dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-1 focus:outline-none focus:ring-0 placeholder:text-slate-400/70 placeholder:font-normal placeholder:text-sm hover:border-slate-400 focus:border-primary-500 dark:focus:border-primary-500  dark:hover:border-slate-700"
                       placeholder="{{ __('admin.placeholders.region_name') }}" required>
            ...
            <div class="mb-2">
                <label for="create_flag_code" class="font-medium text-sm text-slate-600 dark:text-slate-400">{{ __('admin.common.flag_code') }}<span class="text-red-400">*</span></label>
                <input wire:model="createForm.flag_code" type="text" id="create_flag_code" class="form-input w-full rounded-md mt-1 border border-slate-300/60 dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-1 focus:outline-none focus:ring-0 placeholder:text-slate-400/70 placeholder:font-normal placeholder:text-sm hover:border-slate-400 focus:border-primary-500 dark:focus:border-primary-500  dark:hover:border-slate-700"
                       placeholder="{{ __('admin.placeholders.flag_code') }}" required>
                @error('createForm.flag_code')
                    <small class="error block text-red-500 text-xs">{{ $message }}</small>
                @enderror
            </div>
            <div class="mb-2">
                <label for="parent_region" class="font-medium text-sm text-slate-600 dark:text-slate-400">{{ __('admin.common.parent') }}</label>
    ...
    <x-reusable.modal wire:model="showViewModal" :title="__('admin.modal.region_details')" max-width="2xl">
        @if($viewData)
            <div class="space-y-4">
                <div class="bg-slate-50 dark:bg-slate-900 rounded-lg p-4 text-sm border border-slate-200 dark:border-slate-700">
                    <dl class="divide-y divide-slate-200 dark:divide-slate-700">
                        ...
                        <div class="grid grid-cols-3 gap-4 py-3">
                            <dt class="font-medium text-slate-500 dark:text-slate-400">{{ __('admin.common.flag_code') }}</dt>
    ...
    <x-reusable.modal wire:model="showEditModal" :title="__('admin.modal.edit_region', ['id' => $editForm->region?->id])" max-width="2xl">
        <form id="editRegionForm" class="space-y-4" wire:submit="updateRegion">
            ...
            <div class="mb-2">
                <label for="edit_flag_code" class="font-medium text-sm text-slate-600 dark:text-slate-400">{{ __('admin.common.flag_code') }}<span class="text-red-400">*</span></label>
            ...
            <div class="mb-2">
                <label for="edit_parent_region" class="font-medium text-sm text-slate-600 dark:text-slate-400">{{ __('admin.common.parent') }}</label>
            ...
            <div class="flex items-center justify-end space-x-2">
                <button wire:target="updateRegion" type="submit" class="inline-block focus:outline-none text-blue-500 hover:bg-blue-500 hover:text-white bg-transparent border border-blue-200 dark:border-blue-700 text-sm font-medium py-1 px-3 rounded mb-1">{{ __('admin.common.update') }}</button>
    ...
    <x-reusable.modal wire:model="showBulkStatusModal" :title="__('admin.modal.change_region_status')" max-width="md">
    ...
    @push('scripts')
        <x-admin.swal-listener table-name="regionTable" />
    @endpush
</div>
