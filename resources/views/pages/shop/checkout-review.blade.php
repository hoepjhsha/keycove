        ...
        <section class="rounded-[2rem] border border-black/8 bg-white/90 p-6 shadow-[0_24px_80px_-40px_rgba(0,0,0,0.35)] backdrop-blur dark:border-white/10 dark:bg-gray-900/85">
            <nav aria-label="Breadcrumb">
                <ol class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                    ...
                    <li><span class="transition-colors hover:text-[#D32F2F] dark:hover:text-[#ff8b8b]">Cart</span></li>
                    <li><i class="fa-solid fa-chevron-right text-[10px]"></i></li>
                    <li class="font-semibold text-gray-800 dark:text-gray-200" aria-current="page">Checkout</li>
                </ol>
            </nav>
            ...
            <div class="mt-4 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <h1 class="mt-3 text-3xl font-semibold tracking-tight text-gray-950 dark:text-white">Review your order</h1>
                    <p class="mt-2 max-w-2xl text-sm leading-6 text-gray-600 dark:text-gray-400">Check the selected items, total amount, and payment details before you confirm payment.</p>
                </div>

                <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
                    <div class="rounded-3xl border border-black/8 bg-[#FCF9F4] px-4 py-4 dark:border-white/10 dark:bg-white/5">
                        <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">Selected items</p>
                        <p class="mt-3 text-2xl font-semibold text-gray-950 dark:text-white">{{ $selectedCount }}</p>
                    </div>
                    <div class="rounded-3xl border border-black/8 bg-[#FCF9F4] px-4 py-4 dark:border-white/10 dark:bg-white/5">
                        <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">Subtotal</p>
        ...
        <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
                ...
                @foreach($items as $item)
                    <article class="rounded-[1.75rem] border border-black/8 bg-white/90 p-5 shadow-sm dark:border-white/10 dark:bg-gray-900/85">
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                            <div class="space-y-2">
                                <h2 class="text-base font-semibold text-gray-950 dark:text-white">{{ $item->listing?->display_name ?: ($item->listing?->variant?->product?->name ?? 'Store item') }}</h2>
                                ...
                                <div class="flex flex-wrap gap-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-gray-500 dark:text-gray-400">
                                    <span class="rounded-full border border-black/8 bg-[#FCF9F4] px-3 py-1 dark:border-white/10 dark:bg-white/5">Qty {{ $item->quantity }}</span>
                                    <span class="rounded-full border border-black/8 bg-[#FCF9F4] px-3 py-1 dark:border-white/10 dark:bg-white/5">{{ (int) $item->listing?->stock_count }} keys left</span>
                                </div>
                            </div>

                            <div class="text-left sm:text-right">
                                <p class="text-[11px] font-semibold uppercase tracking-[0.16em] text-gray-500 dark:text-gray-400">Item total</p>
            ...
            <aside class="space-y-4">
                <div class="rounded-[1.75rem] border border-black/8 bg-white/90 p-5 shadow-sm dark:border-white/10 dark:bg-gray-900/85">
                    <h2 class="text-base font-semibold text-gray-950 dark:text-white">Payment summary</h2>
                    ...
                    <p class="mt-4 rounded-2xl bg-[#F6EBD9] px-4 py-3 text-xs leading-6 text-gray-600 dark:bg-white/5 dark:text-gray-400">
                        Keys will be reserved while payment is pending. If payment does not complete within 24 hours, the order is cancelled automatically.
                    </p>
                </div>
                    ...
                    <template x-if="checkoutConfirmOpen">
                        <div class="fixed inset-0 z-[95] flex items-center justify-center">
                            ...
                            <div class="relative mx-4 w-full max-w-sm rounded-[1.75rem] border border-black/10 bg-white p-5 shadow-2xl dark:border-white/10 dark:bg-gray-950">
                                <h3 class="text-lg font-semibold text-black dark:text-white">Confirm checkout?</h3>
                                <p class="mt-2 text-sm leading-6 text-gray-500 dark:text-gray-400">You are about to pay for {{ $selectedCount }} selected item(s).</p>
