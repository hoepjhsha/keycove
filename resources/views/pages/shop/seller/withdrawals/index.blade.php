<div class="space-y-6">
    ...
    <section class="overflow-hidden rounded-[2rem] border border-black/8 bg-linear-to-br from-white via-[#FCF9F4] to-[#F6EBD9] p-6 shadow-[0_24px_70px_-45px_rgba(0,0,0,0.45)] dark:border-white/10 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div class="space-y-3">
                ...
                <div>
                    <h2 class="text-3xl font-semibold tracking-tight text-gray-950 dark:text-white">Withdraw your shop balance</h2>
                    <p class="mt-2 max-w-2xl text-sm leading-6 text-gray-600 dark:text-gray-400">Request a withdrawal to your bank account. Available balance is debited immediately and VNPay confirmation is handled automatically.</p>
                </div>
            </div>
            ...
            <div class="rounded-3xl border border-black/8 bg-white/80 px-4 py-4 shadow-sm backdrop-blur dark:border-white/10 dark:bg-white/5">
                <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">Limits</p>
                <p class="mt-3 text-sm text-gray-600 dark:text-gray-400">Min {{ number_format($minimumWithdrawal, 0, ',', '.') }} VND</p>
                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Max {{ number_format($maximumWithdrawal, 0, ',', '.') }} VND</p>
            </div>
        </div>
    </section>
    ...
    <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <article class="rounded-[1.75rem] border border-black/8 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-gray-900/85">
            <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">Available balance</p>
            <p class="mt-3 text-3xl font-semibold text-gray-950 dark:text-white">{{ number_format($metrics['walletBalance'], 0, ',', '.') }} VND</p>
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">VND ready to withdraw</p>
        </article>

        <article class="rounded-[1.75rem] border border-black/8 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-gray-900/85">
            <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">Holding</p>
            <p class="mt-3 text-3xl font-semibold text-gray-950 dark:text-white">{{ number_format($metrics['walletHolding'], 0, ',', '.') }} VND</p>
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Locked by escrow or pending withdrawals</p>
        </article>

        <article class="rounded-[1.75rem] border border-black/8 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-gray-900/85">
            <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">Shop earnings</p>
            <p class="mt-3 text-3xl font-semibold text-gray-950 dark:text-white">{{ number_format($metrics['sellerEarnings'], 0, ',', '.') }} VND</p>
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Seller share of completed sales</p>
        </article>

        <article class="rounded-[1.75rem] border border-black/8 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-gray-900/85">
            <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">Platform fee</p>
            <p class="mt-3 text-3xl font-semibold text-gray-950 dark:text-white">{{ number_format($metrics['platformFee'], 0, ',', '.') }} VND</p>
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Commission charged by the platform</p>
        </article>
    </section>
    ...
    <div class="grid gap-6 xl:grid-cols-2">
        <section class="rounded-[2rem] border border-black/8 bg-white p-6 shadow-[0_24px_70px_-45px_rgba(0,0,0,0.45)] dark:border-white/10 dark:bg-gray-900/85">
            <div class="flex items-center justify-between gap-4">
                <div>
                    <h3 class="text-lg font-semibold text-gray-950 dark:text-white">Withdrawal request</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Fill in your bank details and amount.</p>
                </div>
            </div>
            ...
            <form wire:submit.prevent="submit" class="mt-5 space-y-4">
                ...
                <div>
                    <label for="bankName" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Bank name</label>
                ...
                <div>
                    <label for="bankCode" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Bank code</label>
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        <label for="bankAccountNumber" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Account number</label>
                    ...
                    <div>
                        <label for="bankAccountName" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Account name</label>
                ...
                <div class="flex items-center justify-between gap-4 pt-2">
                    <p class="text-sm text-gray-500 dark:text-gray-400">Requests are handled automatically through VNPay.</p>
        ...
        <section class="rounded-[2rem] border border-black/8 bg-white p-6 shadow-[0_24px_70px_-45px_rgba(0,0,0,0.45)] dark:border-white/10 dark:bg-gray-900/85">
            <div class="flex items-center justify-between gap-4">
                <div>
                    <h3 class="text-lg font-semibold text-gray-950 dark:text-white">Recent withdrawals</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Latest withdrawal requests and processing status.</p>
                </div>
            </div>
