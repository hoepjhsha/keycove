        ...
        <section class="relative overflow-hidden rounded-[2rem] border border-black/8 bg-linear-to-br from-white via-[#FCF9F4] to-[#F6EBD9] p-5 shadow-[0_24px_80px_-40px_rgba(0,0,0,0.35)] sm:p-6 lg:p-8 dark:border-white/10 dark:from-gray-900 dark:via-gray-950 dark:to-gray-900">
            ...
            <div class="relative grid gap-6 lg:grid-cols-[minmax(0,1fr)_18rem] lg:items-end">
                <div class="space-y-4">
                    <nav aria-label="Breadcrumb">
                        <ol class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                            ...
                            <li class="font-semibold text-gray-800 dark:text-gray-200" aria-current="page">Seller application</li>
                        </ol>
                    </nav>
                    ...
                    <div class="space-y-3">
                        ...
                        <div>
                            <h1 class="text-3xl font-semibold tracking-tight text-gray-950 sm:text-4xl dark:text-white">Become a seller</h1>
                            <p class="mt-2 max-w-2xl text-sm leading-6 text-gray-600 sm:text-base dark:text-gray-400">Submit your shop name and identity details. Once approved, your seller dashboard unlocks automatically.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="rounded-3xl border border-black/8 bg-white/80 px-4 py-4 shadow-sm backdrop-blur dark:border-white/10 dark:bg-white/5">
                    ...
                    <p class="mt-3 text-2xl font-semibold text-gray-950 dark:text-white">Verified</p>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Seller applications require a verified email.</p>
                </div>
            </div>
        </section>
        ...
        <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_20rem]">
                ...
                <article class="rounded-[2rem] border border-black/8 bg-white/90 p-6 shadow-[0_24px_70px_-45px_rgba(0,0,0,0.45)] backdrop-blur dark:border-white/10 dark:bg-gray-900/85">
                    <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                        ...
                        <div class="rounded-3xl border border-black/8 bg-gray-50 px-4 py-4 dark:border-white/10 dark:bg-white/5">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">Current status</p>
                            ...
                            <p class="mt-3 text-sm text-gray-600 dark:text-gray-400">{{ $seller?->kyc_rejected_reason ?: 'Your seller profile will stay here while the admin reviews it.' }}</p>
                        </div>
                    </div>
                </article>
                ...
                @if($seller?->kyc_status === \App\Enums\KycStatus::Approved)
                    <article class="rounded-[2rem] border border-emerald-500/15 bg-emerald-500/8 p-6 shadow-sm dark:border-emerald-400/20 dark:bg-emerald-400/10">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="space-y-3">
                                <h3 class="text-lg font-semibold text-emerald-900 dark:text-emerald-200">Your seller account is approved</h3>
                                <p class="text-sm leading-6 text-emerald-800/90 dark:text-emerald-200/80">You can open the seller dashboard now.</p>
                ...
                @else
                    <article class="rounded-[2rem] border border-black/8 bg-white/90 p-6 shadow-[0_24px_70px_-45px_rgba(0,0,0,0.45)] backdrop-blur dark:border-white/10 dark:bg-gray-900/85">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-gray-950 dark:text-white">Application details</h3>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Fill in the form below. The admin team will review your KYC after submission.</p>
                            </div>
                        </div>
                        ...
                        <form wire:submit="submit" class="mt-6 space-y-5">
                            <div class="grid gap-5 md:grid-cols-2">
                                <label class="block space-y-2 md:col-span-2">
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Shop name</span>
                                    <input type="text" wire:model="shopName" class="w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm shadow-sm focus:border-[#D32F2F] focus:ring-0 dark:border-white/10 dark:bg-gray-950 dark:text-gray-100" placeholder="Your store name">
                                    @error('shopName')
                                ...
                                <label class="block space-y-2">
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">CCCD number</span>
                                    <input type="text" wire:model="cccdNumber" inputmode="numeric" maxlength="12" class="w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm shadow-sm focus:border-[#D32F2F] focus:ring-0 dark:border-white/10 dark:bg-gray-950 dark:text-gray-100" placeholder="12 digits">
                                    @error('cccdNumber')
                                        <p class="text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p>
                                    @enderror
                                </label>
                                ...
                                <label class="block space-y-2">
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Front ID image</span>
                                    ...
                                    @if($cccdFrontImage)
                                        <img src="{{ $cccdFrontImage->temporaryUrl() }}" alt="Front ID preview" class="h-28 w-28 rounded-2xl border border-black/10 object-cover dark:border-white/10">
                                    @endif
                                ...
                                <label class="block space-y-2">
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Back ID image</span>
                                    ...
                                    @if($cccdBackImage)
                                        <img src="{{ $cccdBackImage->temporaryUrl() }}" alt="Back ID preview" class="h-28 w-28 rounded-2xl border border-black/10 object-cover dark:border-white/10">
                                    @endif
                                    @error('cccdBackImage')
                                        <p class="text-xs text-rose-600 dark:text-rose-300">{{ $message }}</p>
                                    @enderror
            ...
            <aside class="space-y-6 lg:sticky lg:top-24 lg:self-start">
                <article class="rounded-[2rem] border border-black/8 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-gray-900/85">
                    <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">Checklist</p>
                    <div class="mt-4 space-y-3 text-sm text-gray-600 dark:text-gray-300">
                        <div class="flex items-start gap-3">
                            ...
                            <p>Use your real shop name.</p>
                        </div>
                        <div class="flex items-start gap-3">
                            <i class="fa-solid fa-circle-check mt-1 text-emerald-500"></i>
                            <p>Upload clear front and back ID images.</p>
                        </div>
                        <div class="flex items-start gap-3">
                            <i class="fa-solid fa-circle-check mt-1 text-emerald-500"></i>
                            <p>Once approved, your seller dashboard unlocks automatically.</p>
                        </div>
                    </div>
                </article>
                ...
                <article class="rounded-[2rem] border border-black/8 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-gray-900/85">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            <p class="text-[11px] font-semibold uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">Portal status</p>
