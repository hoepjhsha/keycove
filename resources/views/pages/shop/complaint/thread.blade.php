    ...
    <div class="mx-auto max-w-7xl space-y-6">
        ...
        <section class="rounded-[2rem] border border-black/8 bg-white/90 p-6 shadow-[0_24px_70px_-45px_rgba(0,0,0,0.45)] backdrop-blur dark:border-white/10 dark:bg-gray-900/85">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div class="space-y-3">
                    <div class="flex flex-wrap items-center gap-2">
                        <span class="rounded-full bg-black px-3 py-1 text-[11px] font-semibold text-white dark:bg-white dark:text-gray-950">{{ $complaint->complaint_code ?? ('Khiếu nại #'.$complaint->id) }}</span>
                    ...
                    <div>
                        <h1 class="text-3xl font-semibold tracking-tight text-gray-950 dark:text-white">Hội thoại khiếu nại</h1>
                        <p class="mt-2 max-w-3xl text-sm leading-6 text-gray-600 dark:text-gray-400">Cuộc trao đổi chung để người mua, người bán và quản trị viên xem bằng chứng và quyết định hướng xử lý.</p>
                    </div>
                </div>
            ...
            <div class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <article class="rounded-2xl border border-black/8 bg-[#FCF9F4] px-4 py-4 dark:border-white/10 dark:bg-white/5">
                    ...
                    <p class="mt-2 text-sm font-semibold text-gray-950 dark:text-white">{{ $complaint->orderItem?->product_name_snapshot ?? 'Sản phẩm' }}</p>
                </article>

                <article class="rounded-2xl border border-black/8 bg-[#FCF9F4] px-4 py-4 dark:border-white/10 dark:bg-white/5">
                    <p class="text-[11px] font-semibold uppercase tracking-[0.16em] text-gray-500 dark:text-gray-400">Đơn hàng</p>
                    <p class="mt-2 text-sm font-semibold text-gray-950 dark:text-white">{{ $complaint->orderItem?->order?->order_code ?? '-' }}</p>
                </article>

                <article class="rounded-2xl border border-black/8 bg-[#FCF9F4] px-4 py-4 dark:border-white/10 dark:bg-white/5">
                    <p class="text-[11px] font-semibold uppercase tracking-[0.16em] text-gray-500 dark:text-gray-400">Người tham gia</p>
        ...
        <div class="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
            <section class="space-y-4 rounded-[2rem] border border-black/8 bg-white/90 p-5 shadow-[0_24px_70px_-45px_rgba(0,0,0,0.45)] backdrop-blur dark:border-white/10 dark:bg-gray-900/85">
                ...
                <div class="rounded-2xl border border-black/8 bg-white p-4 dark:border-white/10 dark:bg-gray-950">
                    <div class="flex items-center justify-between gap-3">
                        <h2 class="text-sm font-semibold text-gray-950 dark:text-white">Hội thoại</h2>
                        <span class="text-[11px] font-semibold uppercase tracking-[0.16em] text-gray-500 dark:text-gray-400">{{ count($messages) }} tin nhắn</span>
                    </div>
            ...
            <aside class="space-y-4 rounded-[2rem] border border-black/8 bg-white/90 p-5 shadow-[0_24px_70px_-45px_rgba(0,0,0,0.45)] backdrop-blur dark:border-white/10 dark:bg-gray-900/85">
                <div class="rounded-2xl border border-black/8 bg-[#FCF9F4] p-4 dark:border-white/10 dark:bg-white/5">
                    <p class="text-[11px] font-semibold uppercase tracking-[0.16em] text-gray-500 dark:text-gray-400">Lưu ý</p>
                    <div class="mt-3 space-y-2 text-sm leading-6 text-gray-600 dark:text-gray-300">
                        <p>Vấn đề của người mua sẽ được giữ tại đây cho đến khi quản trị viên duyệt hoàn tiền hoặc giải ngân.</p>
                        <p>Khiếu nại có người bán tham gia sẽ gồm người mua, người bán và quản trị viên trong cùng hội thoại.</p>
                    </div>
                </div>
                ...
                @if($canReply)
                    <form wire:submit="reply" class="rounded-2xl border border-black/8 bg-[#FCF9F4] p-4 dark:border-white/10 dark:bg-white/5">
                        <div class="space-y-2">
                            <label for="complaint-thread-message" class="text-xs font-semibold uppercase tracking-[0.16em] text-gray-500 dark:text-gray-400">Phản hồi</label>
                            <textarea id="complaint-thread-message" wire:model="replyMessage" rows="5" class="w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm shadow-sm focus:border-[#D32F2F] focus:ring-0 dark:border-white/10 dark:bg-gray-950 dark:text-gray-100" placeholder="Thêm phản hồi, bằng chứng hoặc giải thích..."></textarea>
                            @error('replyMessage')
                        ...
                        <div class="mt-4 space-y-2">
                            <label for="complaint-thread-attachments" class="text-xs font-semibold uppercase tracking-[0.16em] text-gray-500 dark:text-gray-400">Tệp đính kèm</label>
                ...
                @if($canResolve)
                    <form class="rounded-2xl border border-black/8 bg-[#FCF9F4] p-4 dark:border-white/10 dark:bg-white/5">
                        <div class="space-y-2">
                            <label for="complaint-resolution-note" class="text-xs font-semibold uppercase tracking-[0.16em] text-gray-500 dark:text-gray-400">Ghi chú xử lý</label>
                            <textarea id="complaint-resolution-note" wire:model="resolutionNote" rows="4" class="w-full rounded-2xl border border-black/10 bg-white px-4 py-3 text-sm shadow-sm focus:border-emerald-500 focus:ring-0 dark:border-white/10 dark:bg-gray-950 dark:text-gray-100" placeholder="Nêu lý do nên hoàn tiền hoặc từ chối khiếu nại..."></textarea>
                        ...
                        <div class="mt-4 space-y-3">
                            ...
                            <button type="button" wire:click="releaseFunds" wire:loading.attr="disabled" class="inline-flex w-full items-center justify-center rounded-2xl bg-gray-900 px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-gray-700 disabled:cursor-not-allowed disabled:opacity-70 dark:bg-white dark:text-gray-950 dark:hover:bg-gray-200">
                                Từ chối khiếu nại và giải ngân
                            </button>
                        </div>
                    </form>
                @endif
