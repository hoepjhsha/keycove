<header class="h-16 rounded-b-xl flex items-center justify-between px-4 sm:px-6 lg:px-8 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 z-10 shrink-0">
    <div class="flex flex-1">
        <form class="flex w-full md:ml-0" action="#" method="GET">
            <label for="search-field" class="sr-only">{{ __('admin.common.search') }}</label>
            <div class="relative w-full text-gray-400 focus-within:text-gray-600 dark:focus-within:text-gray-300">
                <div class="absolute inset-y-0 left-0 flex items-center pointer-events-none">
                    <i class="fa-solid fa-magnifying-glass"></i>
                </div>
                <input id="search-field" class="block h-full w-full border-transparent py-2 pl-8 pr-3 text-gray-900 dark:text-gray-100 bg-transparent placeholder-gray-500 focus:border-transparent focus:placeholder-gray-400 focus:outline-none focus:ring-0 sm:text-sm" placeholder="{{ __('admin.placeholders.search') }}" type="search" name="search">
            </div>
        </form>
    </div>
            ...
            <button type="button" @click="open = !open" class="-m-1.5 flex items-center p-1.5" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                ...
                <span class="hidden lg:flex lg:items-center ml-4">
                    <span class="flex flex-col items-start text-left">
                        <span class="text-sm font-semibold leading-tight text-gray-900 dark:text-gray-200" aria-hidden="true">{{ auth()->guard('admin')->user()?->username ?? __('admin.common.user') }}</span>
                        <span class="text-xs font-medium text-gray-500 dark:text-gray-400 mt-0.5" aria-hidden="true">{{ auth()->guard('admin')->user()?->role->label() ?? __('admin.common.status') }}</span>
                    </span>
