...
>
    <head>
        ...
        <title>{{ $__status ?? __('admin.errors.fallback_meta') }}</title>
            ...
            <div class="w-full max-w-md">
                ...
                <div class="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-8 text-center">
                    ...
                    <h2 class="text-2xl font-bold mb-3 text-slate-900 dark:text-white">
                        @if ($__status === 400)
                            {{ __('admin.errors.400.title') }}
                        @elseif ($__status === 401)
                            {{ __('admin.errors.401.title') }}
                        @elseif ($__status === 503)
                            {{ __('admin.errors.503.title') }}
                        @else
                            {{ __('admin.errors.fallback_title') }}
                        @endif
                    </h2>

                    <!-- Error Description -->
                    <p class="text-slate-600 dark:text-slate-400 mb-8">
                        {{ $message ?? __('admin.errors.fallback_description') }}
                    </p>
                ...
                <p class="text-center text-sm text-slate-500 dark:text-slate-500 mt-8">
                    {{ __('admin.errors.footer', ['code' => $__status ?? 500]) }}
                </p>
            </div>
        </div>
    </body>
